...
int bsm, cch, ch, ff, f, num, por, trig, n, kkk[6][64][25], pos[6][64][25], Nsos_array[64][25],
	cluster[25], nn, anti_f[25], jj, jjj, j, x, gg;

int IACT_numb, co, number_of_pixels_cam, pix_number[64][25];

double pedp, sigp, b[64][25], bmp[64][25] /* matrix for amps from out_file */, ped[64][25],
	sig[64][25], e[25][64] /*single photoelectron amplitude in ADC codes*/,
	sens[25][64] /*relative sensitivity of pixel*/, gain, k_adc, ecode,
	rel_sens, time_0 = 1, event_unix_time = 0;
double edge1, edge2;
string str, srr[25], timet, ped_folder[4] = {"peds", "peds.m3s", "peds.mediana", "peds_median_my"},
	data_path, out_data_path, hillas_table_name, clean_out_name, param_wobble_path, cleaning_type;

double hour, minute, sec, mksec, mlsec, nsec, time0, x_pos[64][25], y_pos[64][25], tim_start = 0,
	tim_end = 0, event_delay;

map<int, string> calendar = {{1, "jan"}, {2, "feb"}, {3, "mar"}, {4, "apr"}, {5, "may"}, {6, "jun"},
...
int main(int argc, char **argv)
{
	...
	int q;  // bad variable, need to remove
							...
							else
							{
								...
								for (int count = 0; count < 25; count++)
								{
									...
									for (int coun = 0; coun < 64; coun++)
									{
										...
										background_marker[coun][count] = 0; // background_marker[i][j] will be set to 1 for image and boundary pixels
									}
								}
								for (int q = 1; q <= n; q++) // q is iterator through clusters (not cluster number!)
								{
									...
									// cout << str << endl;
									ist >> f >> num >> timet; // f is cluster_number from file, num is event_number
									// cout << timet << endl;
									if (q == 1)
									{
										time_cam t;
										...
										{ // only very first line in the file
											...
											time_0 = event_unix_time; // time_0 unix time of first event in the file
										}
									}
									cluster[q] = f;
								...
								for (f = 1; f <= 25; f++)
								{
									...
									for (int sc = 0; sc < 64; sc = sc + 2)
									{
										...
										{																					   // vector<vector<double> > vector_background( number_of_pixels_cam, vector<double> (0));
											...
											{
												vector_background[pix_number[sc][f]].push_back(bmp[sc][f]); // vector_background will have all background signal, the order of pixels in vector_background is from order of rows from neighbors file "neighbours.IACT01"
											}
											bmp[sc][f] = 0;
										}
									}
								...
								for (int count = 0; count < 25; count++)
								{
									for (int coun = 0; coun < 64; coun += 2)
									{
										if (bmp[coun][count] != 0)
										{
											vector_pixel[0].push_back(count);			   // first vector is cluster_number
											vector_pixel[1].push_back(int(coun / 2));	   // second vector is channel_number/2
											vector_pixel[4].push_back(bmp[coun][count]);   // third vector is signal amp (already divided by (e*sens))
								...
								// i+1 is file_number, num is event_number
								event.set_event(i + 1, num, event_unix_time, nsec_time, vector_pixel);
								// event.ccd_id(vector_ccd);

								// yel lines:
								/*if (!(y_set_times.find(time_0) != y_set_times.end()))
								{
									y_set_times.insert(time_0);
									y_vector_times.push_back(time_0);
								}
								cout << "\n\n\n";
								cout << setprecision(10) << "lets look at time_0   : " << time_0 << endl;
								cout << setprecision(10) << "and event.unix_time   : " << event.unix_time << endl;
								cout << setprecision(10) << "time_0 + 12.          : " << time_0 + 12 << endl;
								cout << setprecision(10) << "event.unix_time-time_0: " << event.unix_time-time_0 << endl;
								cout << "\n\n\n";*/
...
void read_config_file(string config_file_name) 
{
	...
	ist700 >> junk_str >> junk_str >> junk_str >> junk_str >> junk_str 
		>> save_background; // make clean and background files 1
