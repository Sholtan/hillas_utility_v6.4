...
public:
	...
	double source_x;
	double source_y;
	double star_x;  // needsclarification   
	double star_y;  // needsclarification
	...
	void set_event(int por, int event_number, double tim, string nst, vector<vector<double>> pixel_parameter)
	{
		...
		{
			pixel_cluster.push_back(pixel_parameter[0][i]); // first vector is cluster_number_
			pixel_number.push_back(pixel_parameter[1][i]);	// second vector is channel_number
			pixel_amp.push_back(pixel_parameter[4][i]);		// fifth vector is signal amp (already divided by (e*sens))
			pixel_x.push_back(pixel_parameter[2][i]);		// third vector is x position of pixel
			pixel_y.push_back(pixel_parameter[3][i]);		// fourth vector is y position of pixel
		}
		number_of_pixels = pixel_amp.size();
	}

	// ccd_data: [0] unix_time, [1] error_deg, [2] tel_ra, [3] tel_dec, [4] tel_az, [5] tel_el,
	...
	int set_ccd_parameters(int ccd_id /*always = 0*/, vector<vector<double>> ccd_data /*pointing data*/)
	{
		...
		{
			delta1 = abs(unix_time - ccd_data[0][i]); // difference between event unix time and wobble[time column (0)][row]
			// cout << delta << "\t" << delta1 << endl;
			// cout << i << endl;
			if (delta < delta1)
			{				// time column is in ascending order, when next delta1 is bigger than previous, the closest is found
				id = i - 1; // to remember row with closest time
				break;
			}
			delta = delta1;
		}
		...
		//	break;
		// }
		ccd_unix_time = ccd_data[0][id]; // local
...
// event.star_correction(amps_mtrx_s_ev, mtrx_neigh_cls_n, mtrx_neigh_chnl_n);
	void star_correction(double amps_mtrx_s_ev[64][25], int mtrx_neigh_cls_n[6][64][25], int mtrx_neigh_chnl_n[6][64][25])
	{
		star = 0;
		for (int k = 0; k < pixel_amp.size(); k++)  // loop over pixels
		{
			if (sqrt(pow(pixel_x[k] - star_x, 2) + pow(pixel_y[k] - star_y, 2)) <= 1.5)  // how many pixels is that??? 
			{
				//cout << "clstr: " << pixel_cluster[k] << ", chnl: " << pixel_number[k] << ", their x and y: " << pixel_x[k] << "  " << pixel_y[k] << endl;
				int sos = 0;
				for (int ss = 0; ss < 6; ss++)
				{
					if (mtrx_neigh_chnl_n[ss][pixel_number[k]][pixel_cluster[k]] > 0 && 
						mtrx_neigh_cls_n[ss][pixel_number[k]][pixel_cluster[k]] > 0 &&
						amps_mtrx_s_ev[ mtrx_neigh_chnl_n[ss][pixel_number[k]][pixel_cluster[k]] ][ mtrx_neigh_cls_n[ss][pixel_number[k]][pixel_cluster[k]] ] > 0)
					{
						sos++;
					}
				}
				if (sos > 1)
				{
					pixel_amp[k] = pixel_amp[k] / sos;  // needsclarification
					amps_mtrx_s_ev[pixel_number[k]][pixel_cluster[k]] = pixel_amp[k] / sos;
				}
				else
				{
					pixel_amp[k] = 0;
					amps_mtrx_s_ev[pixel_number[k]][pixel_cluster[k]] = 0;
				}
